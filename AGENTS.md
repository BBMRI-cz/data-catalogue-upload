...
`data-catalogue-upload` is a **.NET solution** for the data-catalogue sync system. The solution is `DataCatalogue.slnx` at the repo root. The two services:
...
```
DataCatalogue.slnx  Directory.Build.props  Directory.Packages.props  global.json
src/
...
- **Clean Architecture + DDD.** Domain holds aggregates (`Patient`, `PatientAggregate`), value objects, invariants, and **domain services** (biobank `IBiobankCleaningService`; uploader `ISyncPlanner` + `IFingerprintCalculator`). Domain has no I/O and no framework dependencies.
...
- **Pipeline behaviors** (`*.Application/Behaviors`): `LoggingBehavior` and a FluentValidation `ValidationBehavior` that short-circuits to an `ErrorOr` validation error.
...
dotnet test DataCatalogue.slnx                      # all unit + integration tests
dotnet format DataCatalogue.slnx --verify-no-changes   # lint/format (drop the flag to apply)
...
- **Handlers return `ErrorOr<T>`**; don't throw for expected failures. Add a FluentValidation `AbstractValidator` per command/query that needs input validation.
...
```bash
dotnet format DataCatalogue.slnx --verify-no-changes
dotnet build DataCatalogue.slnx -c Release
