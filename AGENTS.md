...
- **uploader** (`src/Uploader`) - a scheduled, one-shot sync job. For each patient it reads from four source APIs (biobank, radiology, sequencing, WSI), aggregates the data into one FAIR Genomes-shaped patient record, compares it against fingerprints stored in PostgreSQL, and saves/deletes rows in a **MOLGENIS EMX2** catalogue over GraphQL. The contract, verified against the live instance, is [`docs/catalogue-api-contract.md`](docs/catalogue-api-contract.md) - read it before touching the gateway.
...
EF Core (Npgsql) + typed `HttpClient`s. Domain `PatientAggregate` + `Sample`/sequencing/WSI/radiology value objects and the `*SyncState` types; `FingerprintSyncPlanner` decides CREATE/UPDATE/SKIP/DELETE. CQRS: `RunCatalogueSyncCommand`. **Each source API serves its own vocabulary and the uploader translates** - one mapper per source in `Application/Mapping/`, over DTOs in `Application/Dtos/` whose property names mirror the source's response records (both sides use `SnakeCaseLower`, so matching names are what makes the wire keys line up; `BiobankContractParityTests` and `SequencingContractParityTests` guard that, each against a recorded response under `tests/Uploader.IntegrationTests/TestData/`). The biobank's `p_tnm`, `morphology`, counts and the rest have no value-object slot yet and are dropped deliberately - each mapper names what it drops and why - and a biobank code becomes an ontology term in `CatalogueVocabulary`, which only maps a code when a term genuinely means the same thing - EMX2 enforces ontology columns as foreign keys, so an invented term fails the whole row. **Sequencing is the one source whose response is a nested list**: `samples[] -> runs[]`, flattened into one `SamplePreparation` per pair on `SequencingAggregate.Preparations`, so a resequenced sample loses nothing; an unknown predictive number answers `200` with an empty `samples`, which yields no aggregate and no failure. Its drops are the run-sample's `files` (FAIR Genomes has no file inventory, and v2 removed even the data-location columns v1 had, so an analysis output survives only as its `DataFormatsStored`), every panel field but `genes`, and `sample_type`/`analysis_type`/`id_scheme`/`sample_index`/`instrument_id`/`workflow`; the run statistics FAIR does not name (cluster counts, density, yield, flowcell, completion status) are packed into `OtherQualityMetrics` by `SequencingMapping`, which also derives the FAIR identifiers. A patient is uploaded only when `PatientCatalogueData.IsUploadEligible`: consented **and** carrying at least one sample. **A source that is absent must not cost the run**: `ISourceDataGateway` returns `ErrorOr<T>` and never throws, a source whose URL is blank is not deployed and is never contacted at all (radiology and WSI default to blank), and the summary counts an unreachable service (`SourceUnavailable`) apart from an unreadable payload (`Failed`). Host `Uploader.Host` applies migrations, runs the sync, prints a JSON summary, and exits `0` (no failures) or `1`. **Identifiers are pseudonymized on the way out**, in `CatalogueMapper` (`Application/Mapping/`), which maps the domain onto the EMX2 rows in `Application/Dtos/Catalogue/` that `ICatalogueGateway` writes - aggregates and sync state keep the real ids, payloads carry pseudonyms, and the type says which side a value is on. Only the patient and sample ids are minted (`IPseudonymMap`/`PseudonymStore`, the `pseudonym` table, prefix `PSEUDONYM_PREFIX`): the clinical, consent and biospecimen identifiers all derive from one of those two via `BiobankMapping`, and the whole sequencing chain is already pseudonymous because `SequencingMapping` derives it from the sequencing API's `sample_id`, which is the run tree's `mmci_predictive_<uuid>` folder name. **The pseudonymizer's mapping files are never read.** WSI and imaging-study upserts return `Catalogue.NotPseudonymized` rather than publishing real bioptic/accession numbers, since no source fills them yet. `CataloguePayloadLeakTests` is the guard; [`docs/pseudonymization.md`](docs/pseudonymization.md) shows the mapping per catalogue table. **One aggregate is several EMX2 rows**, so `ICatalogueGateway` has a method per aggregate rather than per table and owns the write order (patient = `Personal` -> `IndividualConsent` -> `Clinical`); deletes run that backwards, child before parent, because EMX2 refuses to delete a row another row still references - which is what stops a removed patient leaving orphaned samples behind. **A row is deleted only when its source said it was gone**: an aggregate missing because a source did not answer is not an aggregate that was withdrawn, so `PatientCatalogueData.SequencingComplete` and its siblings tell `FingerprintSyncPlanner` which sources reported and it leaves the rest alone - otherwise a network blip deletes published data. A patient who stops being `IsUploadEligible` (consent withdrawn, last sample gone) is **deleted rather than skipped**, ordered last after their own children, and takes everything with them regardless of which sources answered. `Emx2Schema` reads the live column layout and ontology terms once per run, wraps references into the nested key objects EMX2 wants, and drops a term the ontology does not have rather than failing the row. Config via env vars (`POSTGRES_*`, `BIOBANK_API_URL`, `SEQUENCING_API_URL`, the blank-by-default `RADIOLOGY_API_URL`/`WSI_API_URL`, `CATALOGUE_API_URL`/`_SCHEMA`/`_TOKEN`/`_STUDY_ID`/`_STUDY_NAME`, `PSEUDONYM_PREFIX`); see `UploaderOptions`. **The catalogue token comes from `.env`, which is git-ignored** - copy `.env.example`.

**Deployment is three stacks, one compose file each**: `compose.biobank.yml`, `compose.sequencing.yml` and `compose.uploader.yml`. The two source stacks are pinned to a machine by what they read (`/home/mou/patient_data`, `/muni-sc/OrganisedRuns`) because a bind mount resolves only on the host running the container - point one at a path the host lacks and Docker makes an empty directory rather than failing. The uploader reads no files and goes anywhere. Run all three on one host and they are still three projects on three networks, so container names do not resolve between them and the uploader addresses sources as `host.docker.internal`. Ports bind to loopback by default: neither source API authenticates anything and `/patients` serves real identifiers. [`docs/deployment.md`](docs/deployment.md) is the runbook.
