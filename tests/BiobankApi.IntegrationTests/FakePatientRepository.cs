...
/// <summary>In-memory <see cref="IPatientRepository"/> so the API can be tested without a database.</summary>
internal sealed class FakePatientRepository : IPatientRepository
{
    private readonly IReadOnlyList<PatientAggregate> _patients;

    public FakePatientRepository(IReadOnlyList<PatientAggregate> patients) => _patients = patients;
